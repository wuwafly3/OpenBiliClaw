...
`ContentDiscoveryEngine._evaluate_batch` 在每次 batch 调用前会通过 `_get_negative_exemplars()` 从事件层拉一份「最近真正不喜欢」的标题列表（来自 `soul/negative_exemplars.py` 的 recency-weighted、去重、80 字截断、最多 16 条；装配时丢掉完整网站壳标题如 B 站首页干杯标题，不删事件行），并作为 `negative_examples=` 透传给 `build_batch_content_evaluation_prompt()`：
...
| 本地 GLiNER 实体打标 | ✅ | `gliner_tagger.py` 封装零样本 NER 模型 `gliner-community/gliner_large-v2.5`（Apache-2.0 多语言，可选依赖 `[gliner]`）。`evaluate_claim` 在廉价 LLM 通道之前对未打标候选跑本地推理（标题+简介+正文截断拼接），实体列表写 `discovery_candidates.gliner_entities_json` / `gliner_model` / `gliner_at`；`"[]"` 是已打标但无实体的标记，claim 重试不会重复推理。不调用 LLM、不改教师标签与准入分数；缺包 / 加载失败 / 推理异常一律 fail-open 继续完整评估。`[discovery].gliner_tag_enabled` 默认 `off`，标签表默认 8 个领域类型（上限 30），`gliner_threshold=0.5`、`gliner_max_chars=512`。词切分器 `gliner_word_splitter="auto"` 按语种路由（CJK≥0.2 → jieba，否则 whitespace；实测中文候选实体召回 61%→85%、topic 覆盖 1%→9%，拉丁文多词实体不受影响）。 |
...
| 评估上下文快照 | ✅ | 教师打分时冻结 compact eval 画像 + 负例到 `evaluation_context_snapshots`，候选行只存 digest。upsert 失败不影响评估。历史行空串。API daemon 同一次 `_fill_open_slots` 的 claim worker 共用一份 live snapshot。推荐 UI 不变。 |
...
- `apply_gliner_entity_tags()` 是本地实体打标入口：引擎持有懒加载的 `GlinerEntityTagger`（首次调用才在 worker 线程加载模型，推理同样走线程，不阻塞事件循环）。`_get_gliner_tagger()` 在缺 `gliner` 包或模型加载失败时告警一次并停用；候选行持久化走 `Database.update_discovery_candidate_gliner_tags()`。详见下方「本地 GLiNER 实体打标」行。
