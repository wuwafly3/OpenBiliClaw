...
- **eval_context.py（评估上下文快照）** — 打标时刻的 compact 画像摘要 + recall pool + 负例标题，哈希与 eval cache 的 `profile_digest` / `negative_digest` 一致；不存 `personality_portrait`。`rewrite_snapshot_to_gate_contract` 把含 recent / 壳标题的旧快照改写成当前 gate 切片并重算 digest，供重打标脚本使用，不覆盖旧候选行
