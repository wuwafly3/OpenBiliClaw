...
| `gliner_tag_enabled` | bool | `false` | 本地 GLiNER 零样本实体打标（入池打标阶段）：候选评估 claim 时对标题+简介+正文跑一次本地推理，把实体列表写入 `discovery_candidates` 的 `gliner_entities_json` / `gliner_model` / `gliner_at` 列；不调用 LLM、不改教师标签、廉价通道与准入分数。需要 `[gliner]` 可选依赖（连带 torch），缺包或模型加载失败只告警一次并继续正常评估（fail-open）。首次运行会从 Hugging Face 下载权重，可设 `HF_ENDPOINT=https://hf-mirror.com` 走镜像。API、CLI、OpenClaw 三处装配透传；推荐 UI 不变 |
| `gliner_model_id` | string | `"gliner-community/gliner_large-v2.5"` | GLiNER 模型 id（Apache-2.0 多语言 v2.5）或本地权重路径。空值回退默认 |
| `gliner_labels` | list[str] | `["游戏", "动漫", "影视", "音乐", "人物", "组织", "产品", "地点"]` | 零样本实体标签表；去重保序，上限 30 个（GLiNER 前向类型上限），空列表回退默认。也接受逗号分隔字符串 |
| `gliner_threshold` | float | `0.5` | 实体分数下限，合法范围 `[0.05, 1]`，越界回退默认 |
| `gliner_max_chars` | int | `512` | 送入模型的拼接文本上限字符数，合法范围 `64..2048`，越界回退默认 |
| `gliner_word_splitter` | string | `"auto"` | 词切分器：`auto` 按语种路由（CJK 占比 ≥0.2 的文本走 jieba，拉丁文保持 whitespace——GLiNER 训练用空格分词，中文整句会变成一个 token 导致召回崩塌）；其余取值强制统一。非法值回退 `auto`。jieba 路径需要 `pip install jieba3`，缺包回退 whitespace 并告警一次 |
| `relevance_scorer` | string | `"llm"` | **Gate（准入）** 开关，不是精排 ranker。`"llm"` 只走教师评估（默认）；`"shadow"` 在评估前用纯 numpy 打分，对照日志不改 admission；`"ml"` 可保存但仍观察性运行，**不跳过** `evaluate_batch`（改写后的 S1.5 未过）。不写 `relevance_score`，不改 `PoolCurator`。artifact / 版本 / 缺廉价 tags 一律 fail-open 回 LLM。非法值保存时 422 / `ConfigError`。API、CLI、OpenClaw 透传。推荐 UI 不展示。合同见 [gate/ranker 分离 spec](../plans/2026-08-21-ml-gate-ranker-separation-spec.md) |
...
| `gliner_tag_enabled` | bool | `false` | 本地 GLiNER 实体打标（可选依赖 `[gliner]`）；缺包 fail-open。默认 `off` |
| `gliner_word_splitter` | string | `"auto"` | 分词器路由：`auto` / `whitespace` / `jieba` / `hanlp` / `universal`。默认 `auto` |
| `relevance_scorer` | string | `"llm"` | `"llm"` / `"shadow"` / `"ml"`。这是 discovery **gate**，与尚未落地的 `[recommendation].ranker` 独立。shadow 只旁路打分；ml 暂不跳过评估。非法值拒绝 |
