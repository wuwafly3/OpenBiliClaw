...
- **第一轮新合同 gate 训练（不改默认 scorer）：** `--require-snapshot` + 8-24 relabel JSONL，1147 行 / 23 digest 组 / y1=601 y0=546，特征仍是 `admission-teacher-tags-v1`。OOF AUC **0.702**，FPR≤0.10 选点 0.70：agreement 0.536、FPR 0.038、FNR 0.850；0.50 点 agreement 0.656。对照 8-20 旧合同 870 行 0.50 点 AUC 0.741 / agr 0.700。未过改写后的 S1.5（门槛 0.718）。artifact：`data/ml_artifacts/admission_teacher_v1_20260824_snapshot.json`。不改 `relevance_scorer`。
- **生产 gate 训练只收验证快照行：** `scripts/train_relevance_model.py --require-snapshot` 比 `--require-profile-digest` 更严：digest 对必须能取出 `evaluation_context_snapshots` 且 `digests_match()`，空 digest / 缺失 / 哈希失败 / 仍含 recent 的旧合同行不进矩阵，artifact 写 `row_filter=verified_snapshot`。`--relabel-jsonl` 可在过滤前用 8-24 重打标分数覆盖旧行。`--dry-run` 不拟合。2026-08-24 live：只 `--require-snapshot` kept **229**（empty 6979 / old_contract 918 / mismatch 0）；加上 JSONL 后 kept **1147**（918 覆盖 + 229 live 新合同）。不改 `relevance_scorer`。详见 [docs/plans/2026-08-21-ml-gate-ranker-separation-plan.md](plans/2026-08-21-ml-gate-ranker-separation-plan.md) Task 2。
- **旧 gate 快照去 recent 后重打标：** `rewrite_snapshot_to_gate_contract` 丢掉 `recent_awareness` / `active_insights` / `speculative_interests` 与页面壳标题，重算 digest。`scripts/ml_gate_contract_relabel.py` 用 pinned `openai-4` 按新 digest 组重评 8-20 旧合同行，新分写入 JSONL，不覆盖 `discovery_candidates` 旧分。2026-08-24 实跑：**918** 行 / 17 组 / 四平台（B 站 571、小红书 156、YouTube 106、Twitter 85），74 次 `evaluate_batch`，¥**29.26**（prompt cache 约 71%）。新 y 504/414，相对旧合同准入标签翻转 270 行（29.4%）——证实不能把旧分改 digest 当新标签。训练以 JSONL 为准。详见 [docs/plans/2026-08-23-ml-gate-contract-relabel-spec.md](plans/2026-08-23-ml-gate-contract-relabel-spec.md)。
