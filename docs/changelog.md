...
- **旧 gate 快照去 recent 后重打标：** `rewrite_snapshot_to_gate_contract` 丢掉 `recent_awareness` / `active_insights` / `speculative_interests` 与页面壳标题，重算 digest。`scripts/ml_gate_contract_relabel.py` 用 pinned `openai-4` 按新 digest 组重评 8-20 旧合同行，新分写入 `data/ml_artifacts/gate_contract_relabel_*.jsonl`，不覆盖 `discovery_candidates` 旧分 / digest。`--dry-run` 不调 LLM。详见 [docs/plans/2026-08-23-ml-gate-contract-relabel-spec.md](plans/2026-08-23-ml-gate-contract-relabel-spec.md)。
