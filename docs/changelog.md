...
- **GLiNER2 蒸馏标签描述收敛为短正向版并经 A/B 验证：** `scripts/export_gliner2_training_data.py` 的 `TEMPORAL_DESCRIPTIONS`（`eval_gliner2_temporal_adapter.py` 经 import 复用）与 `scripts/gliner2_language_probe.py` 的 `TEMPORAL_DESC` 改为「短、正向、每标签专属关键词」：`versioned` 保留核心语义「指涉可识别且仍在迭代的具体对象，更新后价值衰减」，`historical` 收敛为「对已闭合事件或过去年代的回顾、考据、档案」，`evergreen` 不带排除从句。A/B 验证（同一 val、三标签子集、零样本无 LoRA）：旧描述 38.8% → 初版长描述 21.1%（排除式从句把「工具 / 产品 / 迭代」等 versioned 关键词灌进 evergreen，evergreen recall 37.7%→15.1% 崩盘）→ 短描述 **39.0%**，evergreen 38.7% 恢复、versioned 30.0%→32.5%。结论：GLiNER2.5 强响应 label description（schema 会原样带进模型 prompt），但只认短正向语义，否定从句无效。新增 `scripts/gliner2_temporal_label_description_ab.py` A/B 探针与 `eval_gliner2_temporal_adapter.py --labels` 子集过滤。改后需重新导出训练数据并重训 LoRA。
- **GLiNER2 教师标签重标 + versioned 审计 + 重训闭环：** 新增 `Database.update_discovery_candidate_temporal_relabel`（只写 `temporal_*` 列、不动 relevance / status / score 的保守重标 sink，守卫：unknown / 低置信 / 非法 mode 保留旧标签）与 `scripts/relabel_gliner2_temporal.py`（复用 `evaluate_content_batch`，支持 `--instance` pin、`--eval-max-tokens` 输出预算、瞬态重试）。用新边界判别式全量重标三类 4376 行（148 次调用 ≈ ¥142.87；388 条 8.9% 改标签：evergreen→historical 163、evergreen→current 75、evergreen→versioned 47…）。随后 `scripts/audit_gliner2_versioned.py` 专项复核 versioned 过度标注（提到具名对象但价值永恒被误标）：410 行中 249 条（66%）→evergreen 213 / →current 33 / →historical 3，只留 129 条真 versioned。`scripts/gliner2_temporal_versioned_diag.py` 输出 versioned 混淆矩阵与训练标签抽样诊断。训练脚本新增 `--oversample LABEL:FACTOR` 按标签翻倍复制对抗失衡；清洗 + versioned:3 / breaking:2 / historical:2 重训产出 `data/ml_artifacts/gliner2_temporal_lora_v4`（val exact **77.4%**，versioned recall 19.5%→**69.2%**、historical 40%→**73.3%**、breaking 90.9%），`eval_gliner2_temporal_adapter.py` 默认 adapter 指向 v4。adapter 仍未接入运行时（实验性）。
- **GLiNER2 temporal LoRA 蒸馏试点通过：** 新增三段式脚本 `scripts/export_gliner2_training_data.py`（教师白名单洗成官方训练 JSONL：5623 行→train 5060/val 563，零泄漏）→ `scripts/train_gliner2_temporal_lora.py`（fastino/gliner2.5-multi-v1 + LoRA r8，CPU 2.4h/3 epochs，adapter 仅 5.3MB，Windows 下需 num_workers=0）→ `scripts/eval_gliner2_temporal_adapter.py`。验证集 temporal 精确匹配 **零样本 30.0% → 78.5%**（zh 78.0% / en 81.9%），超过全猜 evergreen 的 66.9% 基线 +11.6pp，breaking recall 90.9%；训练脚本支持 GPU（RTX 5060 8GB 需 torch cu130 构建；bf16 + batch4 峰值 7.1GB，全量 3 epochs 仅 13 分钟、11× 加速，adapter 质量反升：val 78.5%→**81.2%**，versioned recall 17.5%→40.0%）。稀有类仍受类别不平衡影响（versioned 40.0% / historical 31.6%）。artifact 在 gitignored `data/ml_artifacts/gliner2_temporal_lora/`，未接入运行时。
...
- **锁定 explore 去教师：** explore 是第一个可完全去掉教师 LLM 的决策点：质量底线用 gate 灰区下沿（p 暂定 >0.3–0.4），探索价值 = 投机兴趣挂载命中（复用 cosine≥0.45）或高多样性（取反已有疲劳余弦）；流量池 = 灰区 ∩（投机 ∪ 多样）。默认路径暂不改代码。详见 [docs/plans/2026-08-21-ml-gate-ranker-separation-spec.md](plans/2026-08-21-ml-gate-ranker-separation-spec.md) Phase 4。
