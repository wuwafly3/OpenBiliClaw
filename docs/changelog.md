...
- **教师自一致性探针（agreement 天花板）**：新增 `scripts/ml_teacher_self_consistency_probe.py`，对教师白名单按 **精确** `teacher_model == openai/deepseek-v4-flash` 过滤后，用 pinned 实例 `openai-4` 复测完整 `evaluate_batch`（`openai_compatible` 与 `deepseek-v4-flash-thinking` 不混入；配额失败不回落到 openai-2/3）。只记 `llm_usage`，不回写 `discovery_candidates` 分数。2026-08-19 live：白名单 2222、丢掉 compatible 787、exact 1435；90 条样本自洽 **agreement 0.711（64/90）**，FPR 0.170 / FNR 0.459，Spearman ρ 0.678，对照 ML@0.70=0.699、ML@0.50=0.744、S1.5=0.90——当前教师噪声已贴着 ML 一致率，0.90 不是特征缺口。测量项，非合入门槛。详见 [docs/plans/2026-08-19-ml-teacher-self-consistency-probe.md](plans/2026-08-19-ml-teacher-self-consistency-probe.md)。
...
- **Wave 1 教师数据准入模型初训**：不必等廉价 tags 覆盖全量白名单。新增 `scripts/train_relevance_model.py` 与 `[ml]` extra（`numpy` / `scikit-learn`）：用教师 `llm_score_raw` 按 S1.1 打二分类标签，教师 `topic_group` / `style_key` / `temporal_class` 作为廉价通道的 **oracle 特征**（artifact 标明 `tags_source=teacher_oracle`，不是运行时 `tag_channel_*`）。输出 gitignored 权重 / 特征名 / 指纹 / OOF 的 S1.5 测量项。不改 `[discovery].relevance_scorer`，不上推理路径。2026-08-19 live 2955 行（provenance 2222 / legacy 733，y1=1587 / y0=1368）：OOF AUC **0.825**、Brier **0.169**；FPR≤0.10 选点 threshold=0.70 时 agreement 0.699 / FNR 0.474；0.50 点 agreement 0.744 / FPR 0.275 / FNR 0.240。分平台 AUC：youtube 0.944 / bilibili 0.853 / twitter 0.815 / xiaohongshu 0.774（未过 0.80）。joined audit 无共享 `profile_digest`，本次按逐行 GroupKFold。S1.5 仍是测量项，不是合入门槛。
